(function () {
	...
	function startPlugin() {
		...
		Follow.get(Type.de([102, 117, 108, 108]), function (e) {
			if (Type.co(e)) {
				Follow.skodf(e);
				// === ПОЧАТОК ВИПРАВЛЕННЯ (Фікс темного першого фону) ===
				var fixOpacity = function() {
					var $render = e.object.activity.render();
					var $bg = $render.find(".full-start__background");
					if ($bg.length) {
						// Зупиняємо стандартну анімацію Lampa і ставимо яскравість на максимум
						$bg.stop(true, true).css("opacity", "1");
					}
				};
				fixOpacity(); // Викликаємо одразу
				setTimeout(fixOpacity, 300); // Страховка, якщо Lampa підвантажує фон із затримкою
